  ...
  return (
    <HomeLayout>
      <div className="w-full overflow-hidden shadow-xl md:max-w-md md:rounded-2xl md:border md:border-gray-200">
        <div className="relative flex flex-col items-center justify-center border-b border-gray-200 bg-white px-4 py-6 pt-8 text-center dark:border-gray-500 dark:bg-gray-800 dark:text-white md:px-16">
          <h3 className="font-display text-2xl font-bold">Sign In</h3>
        </div>
        <div className="relative min-h-[35px] bg-gray-50 dark:bg-gray-900">
          {response && (
            <div
              className="bg-rose-600 py-2 px-6 text-center text-sm text-rose-900"
              role="alert"
            >
              Error: {response}, are you using correct sign in method?
            </div>
          )}
        </div>
        <div className="relative flex flex-col space-y-4 bg-gray-50 px-4 pb-12 pt-2 dark:bg-gray-900 md:px-16">
          ...
          {PROVIDERS.map((provider) => (
            <button
              ...
              } relative flex h-10 w-full items-center justify-center space-x-3 rounded-md border text-sm shadow-sm transition-all duration-75 focus:outline-none`}
              onClick={() => {
